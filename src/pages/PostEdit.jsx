...
export default function PostEdit() {
  ...
  if (loading) return <div style={{ textAlign: 'center', padding: '80px', color: '#9B7080' }}>불러오는 중... 🐾</div>

  return (
    <div className={styles.page}>
      <header className={styles.header}>
        <div className={styles.headerInner}>
          <Link to={`/posts/${id}`} className={styles.backBtn}>← 돌아가기</Link>
          <span className={styles.headerTitle}>🐾 게시물 수정</span>
        </div>
      </header>
        ...
        <form onSubmit={handleSubmit} className={`card ${styles.form}`}>
          ...
          <div className={styles.formGroup}>
            ...
            <button
              ...
              disabled={imgLoading}
            >
              {imgLoading ? '이미지 가져오는 중...' : '🐕 랜덤 강아지 사진 변경'}
            </button>
            {imageUrl && (
          ...
          <button type="submit" className="btn-primary" disabled={submitting || !title || !content}>
